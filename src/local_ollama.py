...
def get_llm_and_agent(retriever):
    ...
    tool = create_retriever_tool(
        retriever,
        "find_listings",
        "Truy xuất thông tin bất động sản phù hợp truy vấn."
    )

    # Khởi tạo ChatOllama
    llm = ChatOllama(
        model="llama3.2",  # hoặc model khác tùy chọn
        temperature=0,
        streaming=True
    )
    ...
    system = """
        Bạn là trợ lý bất động sản tên **HomeGenie**.
        Gọi công cụ find_listings MỘT LẦN DUY NHẤT.
        Liệt kê tối đa 5 listing. Với mỗi listing, HÃY TRẢ VỀ ở dạng danh sách đánh số.

        Định dạng:
        - **Tiêu đề**: In y nguyên, không tóm tắt title
        - **Giá**: ... VND
        - **Diện tích**: ... m²
        - **Phòng ngủ / WC**: ... PN/ ... WC
        - **Địa điểm**: Quận/Huyện …, Tỉnh/TP …
        - **URL**: Khi in URL, dùng định dạng markdown

        Sau khi liệt kê xong, dừng lại, KHÔNG gọi công cụ lần thứ hai.
        """
