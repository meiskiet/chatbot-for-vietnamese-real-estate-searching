...
def get_retriever(collection_name: str = "data_test") -> EnsembleRetriever:
    ...
    try:
        ...
        documents = [
            ...
            for doc in vectorstore.similarity_search("", k=10)
        ]
...
def get_llm_and_agent(_retriever, model_choice="gpt4") -> AgentExecutor:
    ...
    system = """
        ...
        Khi cần tìm listing hãy dùng công cụ `find_listings`.
        Với mỗi listing, HÃY TRẢ VỀ ở dạng danh sách đánh số:
        ...
        Nếu không có listing thỏa điều kiện, hãy hướng dẫn người dùng lọc lại."""
